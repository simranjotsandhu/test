...
def upload_excel(file, password):
    ...
    if file is None:
        return "⚠️ Please upload an Excel file.", "", "", -1
    df = pd.read_excel(file.name)
    if not {'URL', 'Company Name', 'Tag'}.issubset(df.columns):
        return "⚠️ The Excel file must contain 'URL', 'Company Name', and 'Tag' columns.", "", "", -1
...
def tag_news(index, tag):
    ...
    if not os.path.exists(output_file) or not news_data:
        return "⚠️ No uploaded data found. Please upload a file first.", "", "", -1
...
def show_summary():
    ...
    if not os.path.exists(output_file):
        return "⚠️ No tagging data found."
...
def main():
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as app:
        ...
        with gr.Tab("🏷️ Tag News"):
            gr.Markdown("### 🏷️ Tag News URLs")
